...
import {
  ERR_COMMON_MESSAGE_01,
  ERR_COMMON_MESSAGE_02,
...
const handleRegister = async () => {
  ...
  if (inputUserPassword.value !== inputUserPasswordSecond.value) {
    toast.showErrorToast(
      getMessage(ERR_COMMON_MESSAGE_01, "パスワード", "確認用パスワード")
    );
    return;
  }
  ...
  <div class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white p-8 rounded-lg shadow-lg w-96">
      ...
      <form @submit.prevent="handleRegister">
        <div class="mb-4">
          <label for="user-id" class="text-sm font-medium text-gray-700"
            >ユーザーID (半角英数字)</label
          >
          <input
            type="text"
        ...
        <div class="mb-4">
          <label for="user-name" class="text-sm font-medium text-gray-700"
            >ユーザー名</label
          >
          <input
            type="text"
