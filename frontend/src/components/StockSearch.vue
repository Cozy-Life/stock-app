...
const handleSearchTicker = async () => {
  ...
  stockService(inputTicker.value)
    .then((result) => {
      ...
      displayValue.comparedToThePreviousDay =
        result.data.tickerData.comparedToThePreviousDay;
      displayValue.percentComparedToThePreviousDay =
        result.data.tickerData.percentComparedToThePreviousDay;
      displayValue.yesterdayFinishPrice =
        result.data.tickerData.yesterdayFinishPrice;
      displayValue.startPrice = result.data.tickerData.startPrice;
      ...
      displayValue.dividendPayoutRatio =
        result.data.dividendData.dividendPayoutRatio;
...
<template>
  <div class="grid grid-cols-12 gap-2 lg:gap-4 bg-gray-100">
    <div class="col-span-6 text-2xl md:text-3xl font-bold">株検索</div>
    <input
    ...
    <div class="h-6 lg:h-8 col-span-12 col-start-1 grid grid-cols-12 gap-1">
      <div
        class="col-span-2 lg:col-span-1 text-xs lg:text-sm grid place-items-start items-center"
      >
        検索履歴
      </div>
      <template
        v-for="(historyTicker, index) in latestFiveHistoryTickers"
        :key="historyTicker + '-' + index"
      >
        <button
    ...
    <div
      class="col-span-12 lg:col-span-6 col-start-1 lg:col-start-1 p-2 lg:p-4 grid grid-cols-12 gap-2 bg-white shadow-md rounded-lg"
    >
      <div class="col-span-6 col-start-1 text-xs lg:text-xl">
        {{ `${displayValue.ticker} ${displayValue.sector}` }}
      </div>
      <div class="col-span-12 col-start-1 text-xl lg:text-2xl">
        {{ displayValue.companyName }}
      </div>
      <div class="col-span-5 col-start-1 text-2xl lg:text-3xl font-bold">
        {{ displayValue.stockPrice }}
      </div>
      <div
        :class="[
          'col-span-7 h-10 w-max py-2 px-2 text-xs grid place-items-center text-white rounded-2xl',
          comparedToThePreviousDayBgColor,
        ]"
      >
        前日比： {{ displayValue.comparedToThePreviousDay }} ({{
          displayValue.percentComparedToThePreviousDay
        }}%)
      </div>
    </div>
    <div
      class="col-span-12 lg:col-span-6 col-start-1 lg:col-start-7 p-2 lg:p-4 grid grid-cols-12 gap-2 bg-white shadow-md rounded-lg"
    >
      <div class="col-span-12 col-start-1 text-xl lg:text-2xl">
        株価・株式情報
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        昨日終値
      </div>
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.yesterdayFinishPrice }}
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        始値
      ...
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.highestPrice }}
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        安値
      </div>
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.lowestPrice }}
      </div>
    </div>
    <div
      class="col-span-12 lg:col-span-6 col-start-1 lg:col-start-1 p-2 lg:p-4 grid grid-cols-12 gap-2 bg-white shadow-md rounded-lg"
    ...
    >
      ...
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.baseMonth }}
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        第1四半期
      </div>
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.firstQuarter }}
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        第2四半期
      </div>
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.secondQuarter }}
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        第3四半期
      </div>
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.thirdQuarter }}
      </div>
      <div class="col-span-6 lg:col-span-7 col-start-1 text-sm lg:text-xl">
        第4四半期
      </div>
      <div class="col-span-6 lg:col-span-5 text-sm lg:text-xl">
        {{ displayValue.fourthQuarter }}
      </div>
    </div>
  </div>
